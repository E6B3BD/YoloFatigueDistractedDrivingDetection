...
class FatigueDetecting(object):
    def setupUi(self, parent):
        ...
        # 添加摄像头选项
        parent.cameraChoice = QPushButton("打开摄像头")
        # 创建三个按钮，用于控制视频的开始、打开文件和暂停
        parent.pauseButton = QPushButton("暂停")
        parent.Turn_on_alerts = QPushButton("对称占位")
        parent.openVideoButton = QPushButton("对称占位")
        parent.pauseButton = QPushButton("对称占位")
        # 将下拉框和按钮添加到视频源设置布局中
        ...
        Snack_levels=QHBoxLayout()
         #将零食控件添加到水平控件中
        Snack_levels.addWidget(QLabel("占位"))
